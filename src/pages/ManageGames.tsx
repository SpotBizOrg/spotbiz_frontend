...
function ManageGames() {
  ...
  const handleAddGame = async () => {
    try {
      ...
      if(newGame.title == null || newGame.gameType == null || newGame.developer == null || newGame.description == null || newGame.url== null || newGame.title == '' || newGame.gameType == '' || newGame.developer == '' || newGame.description == '' || newGame.url== ''){
  ...
  const handleUpdateGame = async () => {
    try {
      ...
      if(editGame.title == null || editGame.gameType == null || editGame.developer == null || editGame.description == null || editGame.url== null || editGame.title == '' || editGame.gameType == '' || editGame.developer == '' || editGame.description == '' || editGame.url== ''){
        toast.error("Please fill all fields!");
        throw new Error(`One or more fields are empty`);
      }
  ...
  const handleDeleteGame = async () => {
    try {
      // setLoading(true);
      const response = await fetch(`${BACKEND_URL}/game/delete_game/${deleteGameId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        }
      ...
      <div className="px-12 sm:ml-64 mt-20">
        ...
        <div className="flex flex-col">
          ...
          <div className="text-center text-gray-500">
            {activeTab === 'seasonal' ? 'No seasonal games available.' : 'No regular games available.'}
            </div>
          ) : (
        ...
        {isModalOpen && (
        <Modal show={isModalOpen} onClose={() => setIsModalOpen(false)} size="lg" className="flex items-center justify-center min-h-screen z-40" theme={{
          content: {
            base: "bg-white w-3/4 rounded-lg", 
            inner: "p-5 rounded-lg shadow-lg",
          },
            ...
            <div className="flex flex-wrap space-x-4">
              ...
              <div className="min-w-[196px]">
                ...
                <select
                  id="gameType"
                  className="w-full p-2 border border-gray-300 rounded"
                  onChange={(e) => setEditGame({ ...editGame, gameType: e.target.value })}
              ...
              <div className="flex-1 min-w-[200px]">
                ...
                <TextInput
                  id="gameUrl"
                  placeholder="Game URL"
                  onChange={(e) => setEditGame({ ...editGame, url: e.target.value })}
                  value={editGame.url}
                />
              </div>
       ...
       {isDeleteModelOpen && (
        <Modal show={isDeleteModelOpen} onClose={() => setIsDeleteModelOpen(false)} popup className="flex items-center justify-center inset-2/4 inset-y-1/2 z-40" theme={{
            content: {
              base: "bg-white w-3/4 rounded-lg",
          ...
          <Modal.Body>
            <div className="text-center">
              ...
              <div className="flex justify-center gap-4 pb-6">
                ...
                <Button className="bg-gray-500 hover:bg-gray-600" onClick={() => setIsDeleteModelOpen(false)}>
                  No, cancel
                </Button>
              </div>
            </div>

...
const BusinessTitlenReview: React.FC<BusinessTitlenReviewProps> = ({
  businessName,
  location,
  rating,
  totReviws,
  ...
  if (weeklySchedule) {
    ...
    if (todaySchedule && todaySchedule.isOpen) {
      ...
      if (
        currentSriLankanTime.getTime() >= startTime.getTime() &&
        currentSriLankanTime.getTime() <= endTime.getTime()
      ) {
  ...
  return (
    <div className="bg-white border border-gray-300 p-4 rounded-md shadow-md space-y-2 mb-5 flex justify-between items-center px-10">
      <div>
        <h1 className="text-subheading font-bold text-bluedark">
          {businessName}
        </h1>
        ...
        {updatedStatus === "Open Now" ? (
          <div className="w-40 border border-green-500 text-green-500 font-bold px-4 py-1 rounded-md text-center text-bodysmall mb-2">
            {updatedStatus}
          </div>
        ) : (
          <div className="w-40 border border-red-500 text-red-500 font-bold px-4 py-1 rounded-md text-center text-bodysmall mb-2">
            {updatedStatus}
          </div>
        )}
      </div>
